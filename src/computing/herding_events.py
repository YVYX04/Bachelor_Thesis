...
def identify_herding_events():
    ...
    try:
        merged_df = pd.read_csv(os.path.join(PROCESSED_DATA_PATH, "attention_sample.csv"))
    ...
    merged_df.to_csv(os.path.join(PROCESSED_DATA_PATH, "herding_events_full_attention.csv"), index=False)
    merged_df[merged_df["rh_herd"]].to_csv(os.path.join(PROCESSED_DATA_PATH, "herding_events_only_attention.csv"), index=False)
